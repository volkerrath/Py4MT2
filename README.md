...
This will set up a Python 3.9 environment with all dependencies for aempy. In case you want to keep using python 3.8, corresponding EM38.yml and EM38.txt are available. Don't forget to update also EM38/EM39 regularly, using _conda update --name EM39 (EM38) --all_! There is a replacement for conda, called mamba, which is not only considerably faster, but also better in keeping the environments consistent. It can be installed via conda (i. e., conda install mamba), and has practically the same syntax as the oroginal package manager.
