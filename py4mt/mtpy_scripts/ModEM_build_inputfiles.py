...
import os
# change this path to the path where mtpy is installed
...
edi_list = [
    op.join(
        edipath,
        ff) for ff in os.listdir(edipath) if (
            ff.endswith('.edi'))]
...
do = Data(edi_list=edi_list,
          ...
          error_type_tipper='floor_abs',  # type of error to set in tipper,
                                                # floor_abs is an absolute
                                                # value set as a floor
          error_value_tipper=.03,
          model_epsg=28354  # model epsg, currently set to utm zone 54.
          # See http://spatialreference.org/ to find the epsg code for your
          # projection
          )
do.write_data_file()

# set elevations to zero as we need to ensure the stations are on the
# topography
...
mo = Model(station_locations=do.station_locations,
           ...
           z_mesh_method='new',
           # depth to bottom of core model (padding after this depth)
...
# if the number of air layers is nonzero - topography will be added,
# discretised into that number of cells
mo.add_topography_to_model2(
    r'C:\mtpywin\mtpy\examples\data\AussieContinent_etopo1.asc')
mo.write_model_file(save_path=workdir)
