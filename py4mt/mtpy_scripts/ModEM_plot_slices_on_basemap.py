...
ps = PlotSlices(model_fn,
                data_fn=data_fn,
                model_epsg=28353,
                # epsg used in projection of model grid to ensure correct
                # projection back to lat/long grid
...
for depth in [2e3, 10e3, 20e3]:
    ps.basemap_plot(depth,  # depth to plot (code finds nearest slice)
                    tick_interval=None,
                    # tick interval in degrees, if not provided or None it is
                    ...
                    # option to specify the mesh rotation angle, if rotated
                    # grid was used
                    save=True,
                    # savepath to save figure to. If not provided or
                    # None/False, figure is not saved
                    save_path=savepath
                    )
